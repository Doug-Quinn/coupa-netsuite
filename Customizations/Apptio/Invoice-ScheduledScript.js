...
function scheduled(type)
{
  ...
  if (response.getCode() == '200')
  {
    ...
    for (var i = 0; i < invoiceHeaderNodes.length; i++)
    {
      ...
      {
        nlapiLogExecution('AUDIT', 'Cannot create Vendor Bill as Supplier Number not populated in Coupa', 'Invoice Number = ' + tranid + ' Vendor = ' + nlapiSelectValue(invoiceHeaderNodes[i], 'supplier/name') + ' Coupa Invoice Id = ' + externalid);
        ...
        nlapiSendEmail(-5,
          ...
          'Invoice Number = ' + tranid + ' Vendor = ' + nlapiSelectValue(invoiceHeaderNodes[i], 'supplier/name'));

        continue;
      }

      nlapiLogExecution('AUDIT', 'Processing Coupa Invoice', 'Invoice Number = ' + tranid + ' Vendor = ' + nlapiSelectValue(invoiceHeaderNodes[i], 'supplier/name') + ' Coupa Invoice Id = ' + externalid);
      ...
      {
        nlapiLogExecution('AUDIT', 'Cannot create Vendor Bill as it already exists in Netsuite', 'Invoice Number = ' + tranid + ' Vendor = ' + nlapiSelectValue(invoiceHeaderNodes[i], 'supplier/name') + ' Coupa Invoice Id = ' + externalid + ' Netsuite Vendor Bill id = ' + invoiceexists);
  ...
  if (enable_support_void == 1)
  {
    ...
    if (response.getCode() == '200')
    {
      ...
      for (var i = 0; i < invoiceHeaderNodes.length; i++)
      {
        ...
        nlapiLogExecution('AUDIT', 'Processing Coupa Invoice - VOID in Netsuite', 'Invoice Number = ' + tranid + ' Vendor = ' + nlapiSelectValue(invoiceHeaderNodes[i], 'supplier/name') + ' Coupa Invoice Id = ' + externalid);
        ...
        if (invoiceexists != 'false')
          ...
          nlapiLogExecution('AUDIT', 'Invoice does not exist in Netsuite', 'Invoice Number = ' + tranid + ' Vendor = ' + nlapiSelectValue(invoiceHeaderNodes[i], 'supplier/name') + ' Coupa Invoice Id = ' + externalid);
...
function CreateVendorCredit(invoice)
{
    ...
    if (splitaccounting == 'TRUE')
    {
      for (var i = 0; i < accountallocations.length; i++)
      {
        ...
        if (nlapiGetContext().getSetting('SCRIPT', 'custscript_glactseg'))
        {
          ...
          {
            nlapiLogExecution('ERROR', 'Processing Error - Invalid GL account', 'GL Account =' + accountnumber + ' Invoice Number = ' + nlapiSelectValue(invoice, 'invoice-number') + ' Supplier Name = ' + nlapiSelectValue(supplierNode, 'name'));
            nlapiSendEmail(-5,
              nlapiGetContext().getSetting('SCRIPT', 'custscript_emailaddr_notifications'),
              nlapiGetContext().getSetting('SCRIPT', 'custscript_acccountname') + ' Invoice Integration:Processing Error - Invalid GL account',
              'GL Account =' + accountnumber + ' Invoice Number = ' + nlapiSelectValue(invoice, 'invoice-number') + ' Supplier Name = ' + nlapiSelectValue(supplierNode, 'name'));
            return;
          }
        }
  ...
  {
    nlapiLogExecution('ERROR', 'Processing Error - Unable to create vendor Credit', ' Invoice Number = ' + nlapiSelectValue(invoice, 'invoice-number') + ' Supplier Name = ' + nlapiSelectValue(supplierNode, 'name') + ' Error Description = ' + e.message);
    nlapiSendEmail(-5,
      nlapiGetContext().getSetting('SCRIPT', 'custscript_emailaddr_notifications'),
      ...
      'Invoice Number = ' + nlapiSelectValue(invoice, 'invoice-number') + ' Supplier Name = ' + nlapiSelectValue(supplierNode, 'name') + ' Error Description = ' + e.message);
    return;
  }

  nlapiLogExecution('AUDIT', 'Successfully created vendor Credit', ' Invoice Number = ' + nlapiSelectValue(invoice, 'invoice-number') + ' Supplier Name = ' + nlapiSelectValue(supplierNode, 'name'));
...
function CreateVendorBill(invoice)
{
    ...
    for (var y = 1; y <= nlapiGetContext().getSetting('SCRIPT', 'custscript_customfield_header_count'); y++)
    {
      ...
      if (nlapiGetContext().getSetting('SCRIPT', 'custscript_custfieldheader' + y))
      {
        ...
        if (nlapiGetContext().getSetting('SCRIPT', 'custscript_custfieldheader' + y).split(':'))
        {
          ...
          nlapiLogExecution('DEBUG', 'Header CustomField' + ' ' + y, " custfield0 = " + custfield[0] + " custfield1 = " + custfield[1] + " custfield2 = " + custfield[2] + " custfield3 = " + custfield[3] + " valuetoinsert = " + valuetoinsert);
  ...
  nlapiLogExecution('DEBUG', 'Other Charges', 'Shipping = ' + shippingamount + ' Handling = ' + handlingamount + ' Taxamount = ' + taxamount + ' miscamount = ' + miscamount + ' totalheadercharges = ' + totalheadercharges + ' totalheaderamount = ' + totalheaderamount);
        ...
        if (nlapiGetContext().getSetting('SCRIPT', 'custscript_send_taxcode') == 'T')
        {
          ...
          if (lineleveltaxation == 'false') // no line level tax scenrio - header level tax only
          {
            ...
            if (nlapiSelectValue(invoice, 'tax-code/code'))
            {
              ...
              {
                nlapiLogExecution('ERROR', 'Processing Error - Invalid Header taxcode', 'TaxCode =' + nlapiSelectValue(invoice, 'tax-code/code') + ' Invoice Number = ' + nlapiSelectValue(invoice, 'invoice-number') + ' Supplier Name = ' + nlapiSelectValue(supplierNode, 'name'));
              }
          ...
          else // line level tax scenario
          {
            ...
            if (nlapiSelectValue(invoiceLineNodes[x], 'tax-code/code')) // line level tax and taxcode used
            {
              ...
              {
                nlapiLogExecution('ERROR', 'Processing Error - Invalid taxcode', 'TaxCode =' + nlapiSelectValue(invoiceLineNodes[x], 'tax-code/code') + ' Invoice Number = ' + nlapiSelectValue(invoice, 'invoice-number') + ' Supplier Name = ' + nlapiSelectValue(supplierNode, 'name'));
              }
          ...
          for (var y = 1; y <= nlapiGetContext().getSetting('SCRIPT', 'custscript_customfield_line_count'); y++)
          {
            if (nlapiGetContext().getSetting('SCRIPT', 'custscript_custfieldline' + y))
            {
              ...
              if (nlapiGetContext().getSetting('SCRIPT', 'custscript_custfieldline' + y).split(':'))
              {
                ...
                nlapiLogExecution('DEBUG', 'Line CustomField' + ' ' + y, " custfield0 = " + custfield[0] + " custfield1 = " + custfield[1] + " custfield2 = " + custfield[2] + " custfield3 = " + custfield[3] + " valuetoinsert = " + valuetoinsert);
          ...
          if (nlapiGetContext().getSetting('SCRIPT', 'custscript_custfieldline' + y))
          {
            ...
            if (nlapiGetContext().getSetting('SCRIPT', 'custscript_custfieldline' + y).split(':'))
            {
              ...
              else
              {
                ...
                nlapiLogExecution('DEBUG', 'Line Custom ' + y, 'Coupa field = ' + nlapiSelectValue(invoiceLineNodes[x], custfield[0]) + ' ValuetoInsert = ' + valuetoinsert);
                ...
                if (custfield[2] && nlapiSelectValue(invoiceLineNodes[x], custfield[0]))
                {
                  ...
                  {
                    valuetoinsert = ConvertCoupaDateToNetSuiteDate(nlapiSelectValue(invoiceLineNodes[x], custfield[0]));
                    nlapiLogExecution('DEBUG', 'Line Custom Inside coupatype = date' + y, 'Coupa field = ' + nlapiSelectValue(invoiceLineNodes[x], custfield[0]) + ' ValuetoInsert = ' + valuetoinsert);
                  }
  ...
  {
    nlapiLogExecution('ERROR', 'Processing Error - Unable to create vendor bill', ' Invoice Number = ' + nlapiSelectValue(invoice, 'invoice-number') + ' Supplier Name = ' + nlapiSelectValue(supplierNode, 'name') + ' Error Description = ' + e.message);
    nlapiSendEmail(-5,
      nlapiGetContext().getSetting('SCRIPT', 'custscript_emailaddr_notifications'),
  ...
  nlapiLogExecution('AUDIT', 'Successfully created vendor bill', ' Invoice Number = ' + nlapiSelectValue(invoice, 'invoice-number') + ' Supplier Name = ' + nlapiSelectValue(supplierNode, 'name'));
  Setexportedtotrue(nlapiSelectValue(invoice, 'id'));
}
...
function VoidVendorBill(invoice, id)
{
  ...
  {
    nlapiLogExecution('ERROR', 'Processing Error - Unable to void vendor bill', ' Invoice Number = ' + nlapiSelectValue(invoice, 'invoice-number') + ' Supplier Name = ' + nlapiSelectValue(invoice, 'supplier/name') + ' Error Description = ' + e.message);
    nlapiSendEmail(-5,
      nlapiGetContext().getSetting('SCRIPT', 'custscript_emailaddr_notifications'),
      ...
      'Invoice Number = ' + nlapiSelectValue(invoice, 'invoice-number') + ' Supplier Name = ' + nlapiSelectValue(invoice, 'supplier/name') + ' Error Description = ' + e.message);
  }

  nlapiLogExecution('AUDIT', 'Successfully voided vendor bill', ' Invoice Number = ' + nlapiSelectValue(invoice, 'invoice-number') + ' Supplier Name = ' + nlapiSelectValue(invoice, 'supplier/name'));
...
function SetHeaderChargesasExpenseLine(record, invoice, invoiceline, linecharge, splitaccountNode, isSplit)
{
  ...
  if (nlapiGetContext().getSetting('SCRIPT', 'custscript_glactseg'))
  {
    ...
    {
      nlapiLogExecution('ERROR', 'Processing Error - Invalid GL account', 'GL Account =' + accountnumber + ' Invoice Number = ' + nlapiSelectValue(invoice, 'invoice-number') + ' Supplier Name = ' + nlapiSelectValue(invoice, 'supplier/name'));
      nlapiSendEmail(-5,
        nlapiGetContext().getSetting('SCRIPT', 'custscript_emailaddr_notifications'),
        nlapiGetContext().getSetting('SCRIPT', 'custscript_acccountname') + ' Invoice Integration:Processing Error - Invalid GL account',
        'GL Account =' + accountnumber + ' Invoice Number = ' + nlapiSelectValue(invoice, 'invoice-number') + ' Supplier Name = ' + nlapiSelectValue(invoice, 'supplier/name'));
      return;
    }
  }
      ...
      if (nlapiGetContext().getSetting('SCRIPT', 'custscript_custfieldline' + y))
      {
        ...
        if (nlapiGetContext().getSetting('SCRIPT', 'custscript_custfieldline' + y).split(':'))
        {
          ...
          else
          {
            ...
            nlapiLogExecution('DEBUG', 'Line Custom ' + y, 'Coupa field = ' + nlapiSelectValue(invoiceline, custfield[0]) + ' ValuetoInsert = ' + valuetoinsert);
            ...
            if (custfield[2] && nlapiSelectValue(invoiceline, custfield[0]))
            {
              if (custfield[2] == 'Date')
              {
                ...
                nlapiLogExecution('DEBUG', 'Line Custom Inside coupatype = date' + y, 'Coupa field = ' + nlapiSelectValue(invoiceline, custfield[0]) + ' ValuetoInsert = ' + valuetoinsert);
              }
